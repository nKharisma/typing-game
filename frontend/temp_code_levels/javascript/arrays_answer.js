...
function main() {
    ...
    while (true) {
        ...
        if (action === "add") {
            inventory.push(partName); // Add part to the array
            console.log(`${partName} added to the inventory.`);
        } else if (action === "remove") {
            const index = inventory.indexOf(partName);
            if (index !== -1) {
                inventory.splice(index, 1); // Remove part from the array
                console.log(`${partName} removed from the inventory.`);
            } else {
                console.log(`${partName} not found in the inventory.`);
            }
        } else if (action === "show") {
